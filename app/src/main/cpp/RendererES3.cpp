...
bool RendererES3::init() {
    ...
    for (int i = 0; i < circleEdge; i++) {
        ...
        QUAD[i].pos[1] = radius * sin(i * twicePi / circleEdge);
    }
    */
...
Vertex* RendererES3::mapQUADBuf() {
    glBindBuffer(GL_ARRAY_BUFFER, mVB[VB_INSTANCE]);
    return (Vertex*)glMapBufferRange(GL_ARRAY_BUFFER,
                                     0, sizeof(QUAD),
                                     GL_MAP_WRITE_BIT | GL_MAP_INVALIDATE_BUFFER_BIT);
}

void RendererES3::unmapQUADBuf() {
...
void RendererES3::draw(unsigned int numInstances) {
    ...
    glBindVertexArray(mVBState);
    glDrawArraysInstanced(GL_TRIANGLE_FAN, 0, 12, numInstances);
}
